...
class RewardsCfg:
    ...
    * Contact penalties (thigh, calf, head, front foot)
    ...
    orientation_align = RewTerm(
        func=standup_mdp.orientation_align_velocity_gated,
        weight=3.0,
        params={
            "desired_gravity": DESIRED_GRAVITY_REAR,
            "ang_vel_threshold": 1.5,   # rad/s dead-zone (below = no penalty)
            "lin_vel_threshold": 0.8,   # m/s dead-zone
            "ang_vel_std": 1.0,         # Gaussian decay width beyond threshold
            "lin_vel_std": 0.5,         # Gaussian decay width beyond threshold
        },
    )
    ...
    # Joint velocities: penalize fast joint motion
    # Forces the policy to move slowly
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.003)

    # Joint accelerations: penalize jerk
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7)

    # Joint torques: penalize large forces
...
class RobotStandupRoughEnvCfg(RobotStandupEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.events.reset_prone.params["roll_range"] = (-0.12, 0.12)
        self.events.reset_prone.params["pitch_range"] = (-0.12, 0.12)
